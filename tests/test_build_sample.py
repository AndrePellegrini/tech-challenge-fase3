...
from src.preprocessing.build_sample import (NO_GOLD_STRATUM, anonymize, build_sample,
...
class AnonymizeTests(unittest.TestCase):
    def test_identifiers_are_replaced_and_stable(self):
        series = pd.Series(["11020232", "11020233"])
        first = anonymize(series)
        self.assertNotIn("11020232", set(first))
        self.assertListEqual(list(first), list(anonymize(series)))

    def test_distinct_inputs_give_distinct_hashes(self):
        result = anonymize(pd.Series([str(i) for i in range(500)]))
        self.assertEqual(result.nunique(), 500)

    def test_nulls_are_preserved(self):
        result = anonymize(pd.Series(["123", None]))
...
class BuildSampleTests(unittest.TestCase):
    ...
    def test_identifiers_are_anonymized_but_municipality_is_preserved(self):
